...
Compose предназначен для remote deployment server в Proxmox CT `109`. Домен `crm.lan` создаётся во вкладке Dokploy **Domains** для сервиса `server` и порта `3000`; `ports` и Traefik labels в YAML не добавляются.
